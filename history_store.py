...
def ensure_history_db(db_path: Optional[Path] = None) -> Path:
    ...
    with sqlite3.connect(active_path) as conn:
        ...
        conn.execute(
            ...
            ON usage_samples(metric_group, limit_name, metered_feature, series_label)
            """
        )
    return active_path
...
def insert_history_snapshot(data: Dict[str, Any], db_path: Optional[Path] = None) -> int:
    rows = extract_history_samples(data)
    if not rows:
        return 0
